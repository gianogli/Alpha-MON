...
int main(int argc, char **argv)
{
    ...
    if (ret < 0)
        FATAL_ERROR("Cannot init EAL. Error: %s.", rte_strerror(rte_errno));
    argc -= ret;
    argv += ret;
    ...
    if (ret < 0)
        FATAL_ERROR("Wrong arguments. Quitting.");
...
static void init_port(int i)
{
    ...
    if (ret < 0)
        FATAL_ERROR("Error configuring the port. Error: %s.", rte_strerror(rte_errno));

    /* Check the size of queues */
    ret = rte_eth_dev_adjust_nb_rx_tx_desc(i, &nb_rxd, &nb_txd);
    if (ret < 0)
        FATAL_ERROR("Error adjusting queue numbers. Error: %s.", rte_strerror(rte_errno));
    ...
    for (j = 0; j < nb_sys_cores; j++)
    {
        ...
        if (ret < 0)
            FATAL_ERROR("Error configuring receiving queue. Error: %s.", rte_strerror(rte_errno));

        /* Configure mapping [queue] -> [element in stats array] */
        ret = rte_eth_dev_set_rx_queue_stats_mapping(i, j, j);
        if (ret < 0)
            printf("ANON: [Warning] Error configuring receiving queue stats\n");

        /* Configure tx queue*/
        ret = rte_eth_tx_queue_setup(i, j, TX_QUEUE_SZ, rte_eth_dev_socket_id(i), &tx_conf);
        if (ret < 0)
            FATAL_ERROR("Error configuring transmitting queue. Error: %s.", rte_strerror(rte_errno));

        /* Configure mapping [queue] -> [element in stats array] */
        ret = rte_eth_dev_set_tx_queue_stats_mapping(i, j, j);
        if (ret < 0)
            printf("ANON: [Warning] Error configuring transmitting queue stats\n");
    }

    /* Start device */
    ret = rte_eth_dev_start(i);
    if (ret < 0)
        FATAL_ERROR("Cannot start port. Error: %s.", rte_strerror(rte_errno));
    ...
    if (link.link_status)
        printf("ANON:     Link Up - speed %u Mbps - %s\n",
               (unsigned)link.link_speed, (link.link_duplex == RTE_ETH_LINK_FULL_DUPLEX) ? ("full-duplex") : ("half-duplex\n"));
    else
        printf("ANON:     Port %d Link Down\n", i);

    /* Print RSS support */
    rss_conf.rss_key = rss_key;
    ret = rte_eth_dev_rss_hash_conf_get(i, &rss_conf);
    if (ret == 0)
        printf("ANON:     Device supports RSS\n");
    else
        printf("ANON:     Device DOES NOT support RSS\n");
}
                     ...
                     const char *value)
{
    ...
    else if (strstr(section, "group_") != NULL)
    {
        ...
        else if (strcmp(name, "no_anon_subnet_file") == 0)
        {
            printf("ANON:     %s: %s\n", name, value);
        ...
        {
            printf("ANON:     %s: %s\n", name, value);
            config[ret].payload_drop_enabled = atoi(value);
        }
        /* Protocols */
