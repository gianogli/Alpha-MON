...
int anon_ip_check(struct in_addr adx)
{
    ...
    for (i = 0; i < tot_anon_nets; i++)
    {
        if ((adx.s_addr & anon_net_mask[i]) == anon_net_list[i].s_addr)
        {
            return 1;
        }
    }
...
{
    return match_ipv6_net(adx, anon_net_listv6, anon_net_maskv6, tot_anon_netsv6);
}
...
int no_anon_ip_check(struct in_addr adx)
{
    ...
    for (i = 0; i < tot_no_anon_nets; i++)
    {
        if ((adx.s_addr & no_anon_net_mask[i]) == no_anon_net_list[i].s_addr)
        {
            return 1;
        }
    }
